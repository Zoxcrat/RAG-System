...
def test_maps_rows_to_dicts_preserving_order(monkeypatch):
    ...
    rows = [
        (1, "content one", "doc.txt", 0, 0.12),
        (2, "content two", "doc.txt", 1, 0.34),
    ]

    results = retrieve_mod.retrieve(FakeConn(rows), "q", top_k=2)

    assert results == [
        {"id": 1, "content": "content one", "source": "doc.txt",
         "chunk_index": 0, "distance": 0.12},
        {"id": 2, "content": "content two", "source": "doc.txt",
         "chunk_index": 1, "distance": 0.34},
    ]


def test_result_keys_and_distance_is_float(monkeypatch):
    monkeypatch.setattr(retrieve_mod, "embed_text", lambda q: [1.0])
    rows = [(7, "x", "s", 3, 1)]  # distance given as int -> must be coerced to float

    results = retrieve_mod.retrieve(FakeConn(rows), "q")

    assert set(results[0].keys()) == {"id", "content", "source", "chunk_index", "distance"}
