...
def _format_answer(query: str, columns: list[str], rows: list[tuple]) -> str:
    ...
    return _chat(
        ...
        "distinct-station count for a sub-type IS the per-side number. Present EVERY sub-type row "
        "returned as a breakdown, each with its count AND the supporting wing stations from the "
        "'stations' column (e.g. 'Main: 11 ribs per side at stations 23.6, 57.1, 71.4, ... 208'; "
        "'Trailing-edge: 6 ribs per side'), even if the question emphasizes one sub-type — the "
        "sub-type breakdown with stations is the expected structure. Never add LH and RH columns "
        "together, and never sum the sub-types into a single 'total per side'.\n"
        "- Ranking / 'most common' questions: show the full ranking the rows give (each item with "
        "its measure, highest first), then name the leader — not just the top one in isolation.\n"
